import numpy as np
import faiss

class KNNClassifier:
    def __init__(self, k, distance_metric='l2'):
        self.k = k
        self.distance_metric = distance_metric
        self.X_train = None
        self.Y_train = None

    def fit(self, X_train, Y_train):
        """
        Update the kNN classifier with the provided training data.

        Parameters:
        - X_train (numpy array) of size (N, d): Training feature vectors.
        - Y_train (numpy array) of size (N,): Corresponding class labels.
        """
        self.X_train = X_train.astype(np.float32)
        self.Y_train = Y_train
        d = self.X_train.shape[1]
        if self.distance_metric == 'l2':
            self.index = faiss.index_factory(d, "Flat", faiss.METRIC_L2)
        elif self.distance_metric == 'l1':
            self.index = faiss.index_factory(d, "Flat", faiss.METRIC_L1)
        else:
            raise NotImplementedError
        pass
        self.index.add(self.X_train)

    def predict(self, X):
        """
        Predict the class labels for the given data.
        This is done by finding the neighbors' states and then taking a majority vote.
        Parameters:
        - X (numpy array) of size (M, d): Feature vectors.

        Returns:
        - (numpy array) of size (M,): Predicted class labels.

        """

        # We take the indices of the k nearest neighbors for each test sample
        _, indices = self.knn_distance(X)

        # We get the labels of the k nearest neighbors for each test sample
        neighbor_labels = self.Y_train[indices]

        # Now take a majority vote for each test sample
        predictions = []
        for row in neighbor_labels:
            # row contains the k indices for each test sample
            # We use bincount to count the number of occurrences for each label in a row
            predicted_label = np.bincount(row).argmax()
            predictions.append(predicted_label)

        return np.array(predictions)




    def knn_distance(self, X):
        """
        Calculate kNN distances for the given data. You must use the faiss library to compute the distances.
        See lecture slides and https://github.com/facebookresearch/faiss/wiki/Getting-started#in-python-2 for more information.

        Parameters:
        - X (numpy array) of size (M, d): Feature vectors.

        Returns:
        - D(numpy array) of size (M, k): kNN distances.
        - I(numpy array) of size (M, k): Indices of kNNs.
        """
        X = X.astype(np.float32)
        # Find k nearest neighbors for each test sample: D[i] = distances, I[i] = training indices
        D, I = self.index.search(X, self.k)

        return D, I
